...
while flag_queue
    ...
    adjacent = [row-1, column-1; row-1, column; row-1, column+1; row, column+1; row+1, column+1; row+1, column; row+1, column-1; row, column-1;];
    ...
    for i = 1:size(adjacent, 1)
        ...
        column_adjacent = adjacent(i, 2);
         ...
         if ((G(row_adjacent, column_adjacent) ~= 99) && (visited(row_adjacent, column_adjacent) ~= 1))
            ...
            if (G(row_adjacent, column_adjacent) ~= G(row, column))
